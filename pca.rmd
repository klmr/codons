...
We are going to use only the “canonical CDS”, i.e. the longest one for each
gene.
...
And finally, we choose the longest CDS per gene only.

```{r}
canonical_ccds = ccds_info %>%
    group_by(Gene) %>%
    filter(Length == max(Length)) %>%
    # Because more than one CDS of the same length may exist:
    summarize(CCDS = first(CCDS), Length = first(Length)) %>%
...
```{r}
! any(canonical_ccds$Length %% 3 != 0)
```

Bummer. Log and remove those. And make sure they *are* removed.

```{r}
invalid_cds = canonical_ccds %>% filter(Length %% 3 != 0)
invalid_cds

canonical_ccds = canonical_ccds %>% filter(Length %% 3 == 0)
! any(canonical_ccds$Length %% 3 != 0)
```
...
Now create a table with the codon usage for every gene, based on its canonical
CDS.

```{r}
gene_codon_usage = canonical_ccds %>%
