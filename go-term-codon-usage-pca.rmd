...
plot(cub[, go_ids], pch = '.',
     xlab = sprintf('%s (%d genes)', descriptions[1], go_lengths[1]),
     ylab = sprintf('%s (%d genes)', descriptions[2], go_lengths[2]),
     las = 1)
text(cub[[go_ids[1]]], cub[[go_ids[2]]], labels = cub$AA, pos = 4)
```
...
These seem to be obviously related, just with inverted first PCs. Let’s see how
well the individual PCs correlate.
...
```{r}
pctable_cu_by_aa = do.call(rbind, by(pctable, genetic_code$AA[aa_indices],
                                     function (x) sweep(x, 2, colSums(x), `/`)))
```

This reorders codons by amino acid. Undo the reordering to be on the safe
side, just in case we later refer to it.

```{r go_cub_norm_pca}
rownames(pctable_cu_by_aa) = local({
    x = rownames(pctable_cu_by_aa)
    x = sub('\\w+\\.', '', x)
    # M and W only have a single codon each and were thus not suffixed with the
    # codon name.
    x[x == 'M'] = filter(genetic_code, AA == 'M')$Codon
    x[x == 'W'] = filter(genetic_code, AA == 'W')$Codon
    x
})

pctable_cu_by_aa = local({
    idx = match(rownames(pctable), rownames(pctable_cu_by_aa))
    pctable_cu_by_aa[idx, ]
})

pc_cu_by_aa = prcomp(t(pctable_cu_by_aa))
stopifnot(all.equal(rownames(pc$x), rownames(pc_cu_by_aa$x)))

pc_plot(pc_cu_by_aa, main = 'GO terms by their codon usage bias per amino acid',
        pch = ifelse(is.na(cat), 19, category_symbol[cat]),
        col = ifelse(is.na(cat), '#00000040', category_color[cat]),
        bg = category_color[cat], las = 1)
```
...
```{r go_cu_pc1_gc_cor}
plot(pc$x[, 1], go_gc, xlab = 'PC1 of codon usage', ylab = 'GC bias',
     pch = 16, col = '#00000080', las = 1,
     main = 'Correlation of codon usage and GC bias per GO term')
```

More importantly, is the GC bias also correlated with the codon usage bias?

```{r go_cub_by_aa_pc1_gc_cor}
plot(pc_cu_by_aa$x[, 1], go_gc, xlab = 'PC1 of codon bias', ylab = 'GC bias',
     pch = 16, col = '#00000080', las = 1,
     main = 'Correlation of codon bias and GC bias per GO term')
```

The correlation even exists with pure amino acid usage, to some extent:

```{r go_aa_gc_cor}
plot(pc_aa$x[, 2], go_gc, xlab = 'PC2 of amino acid usage', ylab = 'GC bias',
     pch = 16, col = '#00000080', las = 1,
     main = 'Correlation of amino acid usage and GC bias per GO term')
```
