...
Just out of curiosity, plot the PCA for the per-amino acid codon usage bias.
...
scale_row = function (rows)
    scale(rows, scale = colSums(rows), center = FALSE)

go_cub = by(select(go_table, -Codon, -AA), go_table$AA, scale_row) %>%
    do.call(rbind, .)
...
PCs 1 and 2 are strongly anti-correlated …
