...
* For each gene set, say $\text{DE} = (g_1, g_2, \dots, g_n)$, calculate
  per-gene TE with **matching** tRNA pool
    ...
    $$t_{\text{DE}} = (t_{\text{DE}}(g_1), t_{\text{DE}}(g_2), \dots, t_{\text{DE}}(g_n))$$
...
* Calculate the correlation
    ...
    $$\rho_{\text{DE}} = cor(t_{\text{DE}}, t_{\text{DE}}')$$
...
* If there’s *no* adaptation of TE to cell-specific tRNA pools, then
  $\rho_{\text{DE}}$ is higher than expected by chance.

* But, **how do we quantify “chance”?**

* And: the above was simplified, we have no 1:1 match between “match” and
  “mismatch” tRNA:
